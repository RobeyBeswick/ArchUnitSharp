...
public sealed record GraphSnapshot
{
    ...
    /// <summary>
    /// The snapshot's nodes, one per distinct label the scope's files collapse to. Each access returns
    ...
    /// <summary>
    /// The number of project files the snapshot's scope covers: the sum of the files of every node.
    /// </summary>
    public int FileCount => _nodes.Sum(static node => node.Files.Count);
