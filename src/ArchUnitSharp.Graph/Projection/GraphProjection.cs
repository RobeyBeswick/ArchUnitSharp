...
/// <see cref="ArchUnitSharp.Common.Extraction.Graph"/>. A
/// snapshot is built in four steps: the scope (which files the focus, reachability and dependents
/// restrictions select), the collapse (which label each scoped file maps to), the aggregation (raw
/// dependencies become one edge per label pair, with a count, an external flag and the union of import
/// kinds), and the count (the summary counts the snapshot carries).
/// </summary>
/// <remarks>
/// <para>
...
/// targets are never relabelled — they keep the module name as written.
/// </para>
/// <para>
...
internal static class GraphProjection
{
    ...
    public static GraphSnapshot Build(ArchUnitSharp.Common.Extraction.Graph graph, GraphQueryOptions options)
    {
        ...
        {
            foreach (string target in graph.Edges
                .Where(edge => edge.External && scopeSet.Contains(edge.Source))
                .Select(static edge => edge.Target)
                .Distinct(StringComparer.Ordinal))
            {
                nodes.Add(new SnapshotNode(target, Array.Empty<string>(), external: true));
            }
        }
    ...
    /// <summary>
    /// The files the <c>reachable from</c> restriction selects: the files matching <paramref name="filter"/>
    /// plus everything reachable from them by following internal dependency edges.
    /// </summary>
    ...
    /// <summary>
    /// The files the <c>dependents of</c> restriction selects: the files matching <paramref name="filter"/>
    /// plus everything that can reach them by following internal dependency edges backwards.
    /// </summary>
    ...
    /// root bucket for a root-level file or a depth of zero.
    /// </summary>
