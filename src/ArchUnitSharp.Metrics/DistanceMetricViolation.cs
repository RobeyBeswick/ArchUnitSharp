...
/// <summary>
/// A violation produced by a distance-metric rule: one file whose measured metric value missed the
/// rule's threshold or failed its predicate. Carries the data a report needs and nothing else: which
/// file, which metric, what value it had, and what the rule required.
/// </summary>
/// <remarks>
/// <para>
/// <see cref="File"/> is the offending file's graph identifier. A threshold rule — <c>should be
/// below</c> and friends — sets <see cref="Comparison"/> and <see cref="Threshold"/>; a <c>should
/// satisfy</c> rule sets <see cref="Message"/> instead. <see cref="Value"/> is the measured value in
/// both shapes. It carries <see cref="ViolationKind.Rule"/>, the same kind every rule predicate
/// violation carries.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
public sealed record DistanceMetricViolation : Violation
{
    ...
    /// <summary>
    /// The distance metric that was measured.
    /// </summary>
    public DistanceMetricKind MetricKind { get; }
    ...
    /// <summary>
    /// Creates a violation for a file whose value missed a threshold rule's comparison and threshold.
    /// </summary>
    /// <param name="file">The offending file's graph identifier; must not be <see langword="null"/> or empty.</param>
    /// <param name="metricKind">The distance metric that was measured.</param>
    ...
    public DistanceMetricViolation(
        string file,
        DistanceMetricKind metricKind,
        double value,
    ...
    /// <summary>
    /// Creates a violation for a file whose value failed a <c>should satisfy</c> rule's predicate.
    /// </summary>
    /// <param name="file">The offending file's graph identifier; must not be <see langword="null"/> or empty.</param>
    /// <param name="metricKind">The distance metric that was measured.</param>
    /// <param name="value">The measured value.</param>
