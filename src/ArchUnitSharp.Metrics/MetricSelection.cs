...
/// <summary>
/// One metric over one metrics scope: the PREDICATE-OBJECT of a count-metric rule chain. Built from a
/// <see cref="CountMetrics"/> metric method; its threshold methods complete the rule and each returns
/// the terminal that is checked with <see cref="ICheckable.Check(CheckOptions?)"/>.
/// </summary>
/// <remarks>
/// <para>
/// This type is the metric selection, nothing else: it carries the scope and the <see cref="Metric"/>,
...
/// threshold, or to satisfy a custom predicate — so every rule reads as a sentence: <c>method count
/// should be below 20</c>. The negated mood has no metric analogue: a comparison's negation is another
...
/// it was built from, so a <see cref="MetricSelection"/> value can be stored and reused.
/// </para>
/// </remarks>
public sealed class MetricSelection
{
    ...
    /// <summary>
    /// Creates a selection of one metric over one scope. Callers obtain a
    /// <see cref="MetricSelection"/> from a <see cref="CountMetrics"/> metric method rather than
    /// constructing one.
    /// </summary>
    /// <param name="metrics">The scope the metric is selected over.</param>
    /// <param name="metric">The selected metric.</param>
    internal MetricSelection(Metrics metrics, Metric metric)
    {
        _metrics = metrics;
        _metric = metric;
    }
    ...
    /// <see cref="MetricViolation"/>, and the empty-test guard reports a rule whose subjects matched
    /// nothing.
    /// </summary>
    ...
    public ICheckable ShouldBeBelow(int threshold) =>
        new MetricRule(_metrics, _metric, MetricComparison.Below, threshold);
    ...
    public ICheckable ShouldBeAbove(int threshold) =>
        new MetricRule(_metrics, _metric, MetricComparison.Above, threshold);

    /// <summary>
    /// <c>should be</c>: every subject's metric value must equal <paramref name="threshold"/>. A
    /// subject whose value does not is reported as one <see cref="MetricViolation"/>, and the
    ...
    public ICheckable ShouldBe(int threshold) =>
        new MetricRule(_metrics, _metric, MetricComparison.Equal, threshold);
    ...
    public ICheckable ShouldBeBelowOrEqual(int threshold) =>
        new MetricRule(_metrics, _metric, MetricComparison.BelowOrEqual, threshold);
    ...
    public ICheckable ShouldBeAboveOrEqual(int threshold) =>
        new MetricRule(_metrics, _metric, MetricComparison.AboveOrEqual, threshold);
    ...
    /// <see cref="MetricViolation"/> carrying <paramref name="message"/>, and the empty-test guard
    ...
    public ICheckable ShouldSatisfy(Func<int, bool> predicate, string message)
    {
        ...
        return new MetricRule(_metrics, _metric, predicate, message);
    }
}
