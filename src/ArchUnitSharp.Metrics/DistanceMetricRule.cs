...
/// <summary>
/// The terminal of a distance-metric rule chain: the scope, the <see cref="DistanceMetric"/>, and
/// either a <see cref="MetricComparison"/> plus threshold or a custom predicate plus message. Checked
/// with <see cref="Check(CheckOptions?)"/>, which computes the subjects' measured values and reports
/// each subject that misses the rule as one <see cref="DistanceMetricViolation"/>.
/// </summary>
/// <remarks>
/// <para>
/// This type is built by a <see cref="DistanceMetricSelection"/> threshold method and is the only rule
/// shape the distance section's metric methods produce. Its two modes are exclusive: a threshold rule
/// carries a comparison and a threshold, a <c>should satisfy</c> rule carries a predicate and a
/// message. The assertion layer reads the mode from <see cref="Predicate"/> being non-<see langword="null"/>.
/// </para>
/// <para>
...
/// </para>
/// </remarks>
internal sealed class DistanceMetricRule : ICheckable
{
